/*
 * linkedlist.c - pointer-chasing linked-list microbenchmark
 *
 * Source: github.com/sid-agrawal/cheribsd, cp_benchmarks/pointers/linkedlist.c
 * (branch cherry-picking-merge). Ported to Linux for Eden: dropped
 * <sys/sysctl.h> (FreeBSD-only, was unused - the CHERI-specific sysctl
 * stats printing below is already commented out in the source) and the
 * per-node debug printf()s in the construction/traversal loops (left in by
 * the original author, would flood stdout for any non-trivial num_nodes).
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char* argv[])
{
	...
	printf("Done: %d nodes traversed, construction=%lds traversal=%lds\n",
		count, CDur, TDur);
        /* Print the stats
        printf("{");
        printf("\"Test\": \"%s\",", "ll");
